...
from plugins.dork.services.saver import deduplicate_links, save_unique_links, load_links_from_file
from plugins.dork.services.search_runner import SearchRunner

class Dorking:
    def __init__(self, output_dir="output", max_pages=100, slow_search=False, link_file="collected_links.txt", no_download=True):
        ...
        if os.path.exists(self.link_file):
            self.logger.load_links(self.seen_urls, self.file_metadata)
    ...
    def run(self, query):
        ...
        existing_links = load_links_from_file(self.link_file)
        unique_links = deduplicate_links(existing_links, all_links)
        save_unique_links(self.link_file, unique_links)
